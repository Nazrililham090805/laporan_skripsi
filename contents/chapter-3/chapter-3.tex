...
Hadis-hadis demikian tidak dapat dimodelkan sebagai relasi dalam graf jaringan sanad karena tidak memuat informasi perawi maupun hubungan antar-perawi. Oleh karena itu, seluruh baris dengan nilai \texttt{'No SANAD'} dihapus dari dataset menggunakan filter kondisional. Setelah proses ini, dataset yang tersisa berjumlah 491.428 baris, berkurang sebesar 159.558 baris atau sekitar 24,5\% dari total data awal. Selain itu, karena pembangunan graf memerlukan minimal dua perawi untuk membentuk satu relasi guru--murid, rantai sanad yang hanya memuat satu nama perawi dihapus karena tidak dapat membentuk pasangan periwayatan. Dengan demikian, jumlah rantai sanad yang benar-benar digunakan pada tahap pembangunan graf adalah 487.451 rantai, dan seluruhnya dipastikan memiliki rantai sanad yang dapat diproses pada tahap-tahap selanjutnya.
\end{sloppypar}
...
Dengan demikian, hasil analisis tersimpan secara terpusat di Supabase dan siap diakses oleh \textit{dashboard}. Adapun rancangan rinci kedua tabel, yang mencakup struktur kolom serta diagram relasi antar-tabelnya (ERD), dijelaskan secara tersendiri pada Subbab~\ref{subsubsec:perancangan-basis-data}.
...
\begin{enumerate}
	...
	\item \textbf{Persiapan Dataset.} Pengumpulan dan penyiapan data hadis dari studi Mghari \textit{et al}, khususnya berkas \texttt{sanadset.csv} yang memuat rantai sanad, sebagaimana dijelaskan pada Subbab~\ref{subsec:pra-pemrosesan}.
	\item \textbf{Pra-Pemrosesan Data.} Pembersihan dan normalisasi nama perawi, meliputi penghapusan baris tanpa sanad, penghapusan harakat, normalisasi huruf dan kata, hingga validasi karakter, sebagaimana diuraikan pada Subbab~\ref{subsec:pra-pemrosesan}.
	\item \textbf{Pembangunan \textit{Knowledge Graph}.} Pemodelan rantai sanad menjadi graf berarah berisi simpul perawi dan sisi relasi periwayatan guru--murid, sebagaimana dijelaskan pada Subbab~\ref{subsec:pembangunan-kg}.
	\item \textbf{Analisis Jaringan Sanad.} Perhitungan kelima metrik sentralitas menggunakan pustaka NetworkX untuk mengidentifikasi peran tiap perawi dalam jaringan, sebagaimana dijelaskan pada Subbab~\ref{subsec:analisis-jaringan}.
	\item \textbf{Penyimpanan ke Basis Data.} Hasil analisis diekspor menjadi berkas CSV lalu disimpan ke basis data Supabase agar dapat diakses secara terpusat oleh \textit{dashboard}, sebagaimana dijelaskan pada Subbab~\ref{subsubsec:penyimpanan-basis-data}.
