...
\begin{sloppypar}
Kelima persamaan di atas merupakan definisi matematis dari masing-masing metrik sentralitas, yaitu bentuk perhitungan apabila nilai sentralitas dihitung secara manual. Dalam penelitian ini, perhitungan tersebut tidak dilakukan secara manual, melainkan menggunakan pustaka \textit{NetworkX} yang telah menyediakan implementasi baku untuk setiap metrik, masing-masing melalui fungsi \textit{in-degree centrality}, \textit{out-degree centrality}, \textit{eigenvector centrality}, \textit{closeness centrality}, dan \textit{betweenness centrality}. Pencantuman persamaan matematis ini bertujuan untuk menjelaskan dasar teori dan logika perhitungan di balik setiap metrik, sehingga nilai yang dihasilkan oleh \textit{NetworkX} dapat dipahami maknanya secara konseptual, bukan sekadar diperoleh sebagai keluaran pustaka.
\end{sloppypar}
...
Pengujian sistem dilakukan untuk memastikan setiap fungsi pada \textit{dashboard} berjalan sesuai dengan yang dirancang. Metode pengujian yang digunakan pada penelitian ini adalah \textit{black-box testing}, yaitu pengujian yang berfokus pada fungsionalitas sistem tanpa memeriksa struktur kode internalnya. Pengujian ini dilakukan dengan memberikan masukan (\textit{input}) tertentu pada setiap fitur, lalu membandingkan keluaran (\textit{output}) yang dihasilkan dengan keluaran yang diharapkan. Sebuah fitur dinyatakan valid apabila keluaran yang dihasilkan sesuai dengan keluaran yang diharapkan.
...
\begin{figure}[H]
	\centering
	\includegraphics[height=0.85\textheight]{contents/chapter-3/AlurTugasAkhir.png}
